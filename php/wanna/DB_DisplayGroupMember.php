...
if($userType == "Person"){
	$result = mysql_query("SELECT `nickName`, `profileID` FROM `personprofile` where `profileID` in(SELECT `pofileID` FROM `users` WHERE `userid` in (SELECT DISTINCT(`userid`) from `groupjoinin` WHERE `groupID`=$groupID))");
}else if($userType == "Organization"){
	$result = mysql_query("SELECT `nickName`, `profileID` FROM `organizationfile` where `profileID `in (SELECT `pofileID` FROM `users` WHERE `userid` in(SELECT DISTINCT(`userid`) from `groupjoinin` WHERE `groupID`=$groupID))");
}
