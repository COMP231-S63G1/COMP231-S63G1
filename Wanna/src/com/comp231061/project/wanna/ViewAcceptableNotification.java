...
public class ViewAcceptableNotification extends Activity {
	...
	private String urlDeclineRequest = UserFunctions.URL_ROOT
			+ "DB_DeclineRequest.php";
	private String urlAcceptRequest = UserFunctions.URL_ROOT
			+ "DB_AcceptRequest.php";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_view_acceptable_notification);
    ...
    public void onAcceptClick(View view){	
    	new urlAcceptRequest().execute();
		}

    public void onDeclineClick(View view){	
    	new DeclineRequestTask().execute();
		}
	...
	private class urlAcceptRequest extends AsyncTask<String, Void, String> {
		String message;

		@Override
		protected String doInBackground(String... urls) {
			List<NameValuePair> acceptRequestParams = new ArrayList<NameValuePair>();
			acceptRequestParams.add(new BasicNameValuePair(TAG_SESSIONID,
					sessionID));
			acceptRequestParams.add(new BasicNameValuePair(TAG_USERID,
					userID));
			acceptRequestParams.add(new BasicNameValuePair(TAG_USERTYPE,
					userType));
			acceptRequestParams.add(new BasicNameValuePair(
					TAG_NOTIFICATIONID, notificationID));
			// getting profile info by making HTTP request
			JSONObject json = jsonParser.getJSONFromUrl(
					urlAcceptRequest, acceptRequestParams);
			// json success tag
			success = json.optInt(TAG_SUCCESS);
			message = json.optString(TAG_MESSAGE);			
			return null;
		...
		protected void onPostExecute(String result) {
			Toast.makeText(getApplicationContext(), message,
					Toast.LENGTH_SHORT).show();
			btnNotificationDecline.setEnabled(false);
			btnNotificationAccept.setEnabled(false);
		}
	}
	private class DeclineRequestTask extends AsyncTask<String, Void, String> {
		String message;

		@Override
		protected String doInBackground(String... urls) {
			List<NameValuePair> declineRequestParams = new ArrayList<NameValuePair>();
			declineRequestParams.add(new BasicNameValuePair(TAG_SESSIONID,
					sessionID));
			declineRequestParams.add(new BasicNameValuePair(TAG_USERID,
					userID));
			declineRequestParams.add(new BasicNameValuePair(TAG_USERTYPE,
					userType));
			declineRequestParams.add(new BasicNameValuePair(
					TAG_NOTIFICATIONID, notificationID));
			// getting profile info by making HTTP request
			JSONObject json = jsonParser.getJSONFromUrl(
					urlDeclineRequest, declineRequestParams);
			// json success tag
			success = json.optInt(TAG_SUCCESS);
			message = json.optString(TAG_MESSAGE);
			return null;
